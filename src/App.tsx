...
import { FolderOpen, AlertTriangle, Loader2, HardDrive } from 'lucide-react'
...
function App() {
  ...
  const [scanStats, setScanStats] = useState<{ filesFound: number, directoriesFound: number, totalSize: number } | null>(null)
    ...
    const setupListener = async () => {
      try {
        unlisten = await listen<ScanEvent>('scan-event', (event) => {
          ...
          switch (payload.type) {
            case 'progress':
              console.log('收到进度事件，scanId:', payload.scanId, '已处理:', payload.processed, '总数:', payload.totalEstimated)
              setScanProgress({
                ...
                phase: payload.phase,
                percentage: payload.phase === 'processing' && payload.totalEstimated
            ...
            case 'completed':
              console.log('收到完成事件，scanId:', payload.scanId, '文件:', payload.filesFound, '目录:', payload.directoriesFound)
              const resultsForStats = payload.results ?? []
              const derivedFilesFound = resultsForStats.filter((x) => !x.is_dir).length
              const derivedDirectoriesFound = resultsForStats.filter((x) => x.is_dir).length
              const derivedTotalSize = resultsForStats.filter((x) => !x.is_dir).reduce((s, x) => s + x.size, 0)
              setScanStats({
                filesFound: Number(payload.filesFound ?? derivedFilesFound ?? 0),
                directoriesFound: Number(payload.directoriesFound ?? derivedDirectoriesFound ?? 0),
            ...
            case 'cancelled':
              ...
              console.log('收到取消事件，scanId:', payload.scanId, '当前scanId:', latestScanId)
            ...
            case 'timeout':
              ...
              console.log('收到超时事件，scanId:', payload.scanId, '当前scanId:', timeoutScanId)
              if (payload.scanId === timeoutScanId || timeoutScanId === null) {
                setError(`扫描超时 (${lastTimeoutSeconds}秒)，请尝试扫描较小的目录或增加限制。`)
                setLoading(false)
                setCancelPending(false)
                setCurrentScanId(null)
                setScanProgress(null)
            ...
            case 'error':
              console.log('收到错误事件，scanId:', payload.scanId, '消息:', payload.message)
  ...
  const handleScan = async (path: string, limit: number, minSize: number, timeoutSeconds: number) => {
  ...
  const handleCancelScan = async () => {
    if (currentScanId && !cancelPending) {
      ...
      try {
        ...
        const cancelled = await invoke<boolean>('cancel_scan', { scanId: currentScanId })
        console.log('取消扫描结果:', cancelled)
        if (cancelled) {
          // 设置超时，如果5秒内没有收到取消事件，强制重置状态
          setTimeout(() => {
            console.log('取消扫描超时，强制重置状态，当前scanId:', currentScanIdRef.current)
            // 检查是否已经收到取消事件
  ...
  const confirmDeleteAction = async () => {
    ...
    try {
      await invoke('delete_path', { path: confirmDelete.path })
      // 从列表中移除已删除的项目
  ...
  const headerTotalSize = scanStats && scannedTotalSize > 0 ? scannedTotalSize : displayedSize
  const totalItems = (scanStats?.filesFound ?? 0) + (scanStats?.directoriesFound ?? 0)
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex flex-col items-end gap-1 text-right max-w-[260px]">
            <div className="text-sm text-gray-500 leading-tight">
              总扫描大小: <span className="font-semibold">{formatFileSize(headerTotalSize)}</span>
            </div>
            {scanStats && (
            ...
            {scanStats && (
              <>
                <div className="text-xs text-gray-400 leading-tight">
                  文件: {scanStats.filesFound.toLocaleString()} | 目录: {scanStats.directoriesFound.toLocaleString()}
                </div>
                <div className="text-xs text-gray-400 leading-tight">
          ...
          <div className="lg:col-span-2">
            ...
            {loading ? (
              <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-sm text-gray-500 mt-2">这可能需要一些时间，具体取决于目录大小</p>
              </div>
            ) : files.length > 0 ? (
              <FileList
                files={files}
